...
class IConClassifier(BaseProcessor.BaseProcessor):
    def __init__(self, config, DBP):
        ...
        self.config['ICON_DATASET_PATH'] = '/data/wangruifeng/datasets/DroidBot_Epoch/raw_data/db_universal_set/derived_dataset/13types_train_set/seed_icons_dataset.vec'
        self.config['SEED_IMAGE_DIR'] = '/data/wangruifeng/datasets/DroidBot_Epoch/raw_data/db_universal_set/derived_dataset/13types_train_set/train'
        self.config['ICON_DATASET_INFO_PATH'] = '/data/wangruifeng/datasets/DroidBot_Epoch/raw_data/db_universal_set/derived_dataset/13types_train_set/seed_icons_dataset_info.csv'
        #self.config['ICON_DATASET_PATH'] = self.config['INPUT_DATA_DIR'].replace('input_data/','')+'/seed_icons/seed_icons_dataset.vec'
        #self.config['ICON_DATASET_INFO_PATH'] = self.config['INPUT_DATA_DIR'].replace('input_data/','')+'/seed_icons/seed_icons_dataset_info.csv'
        #self.config['SEED_IMAGE_DIR'] = self.config['INPUT_DATA_DIR'].replace('input_data/','')+'/seed_icons'
    ...
    Example:
        ...
        if operator == 'createds':
            if params  == 'knn':
                self.createDatasetWithKNN(self.extractImageFeature)
            elif params == 'clustering':
                self.createDatasetWithClustering(self.extractImageFeature)
    ...
    def createDatasetWithClustering(self):
        pass


    def KNN_old(self,X_train,y_train,X_test,k=10):
        # 遍历test_list,在train_list中计算距离
        y_test = []
        for test_index in range(X_test):
            distance_list = []
            for train_index in range(X_train):
                distance_list.append([np.linalg.norm(X_test[test_index]-X_train[train_index]),y_train[train_index]])
            distance.sort(key=lambda x: x[0])
            y_test[test_index] = Counter(np.array(distance)[:k,1]).most_common()[0][0]

        return y_test

    def KNN(self,train_list,train_info_list,test_list,test_info_list,k=5):
        #注意,这里的train_list指作为KNN标准的训练集，与外界无关
        train_list = np.array(train_list)
        test_list = np.array(test_list)
        train_info_list = np.array(train_info_list)
        test_info_list = np.array(test_info_list)
        print('train_list.shape:,test_list.shape:',train_list.shape,test_list.shape)
        print(train_list[0][-3:],'\n',test_list[0][-3:])
        print(train_info_list[0][-3:],'\n',test_info_list[0][-3:])
        # 遍历test_list,在train_list中计算距离
        for test_index in range(len(test_list)):
            distance_list = []
            for train_index in range(len(train_list)):
                distance_list.append([np.linalg.norm(test_list[test_index]-train_list[train_index]),int(train_info_list[train_index][-1])])
            distance_list.sort(key=lambda x: x[0])
            #print(Counter(np.array(distance_list)[:k,1]).most_common())
            counter = Counter(np.array(distance_list)[:k,1]).most_common() #=> [(1,3),(2,1),(4,1)]
            print(test_info_list[test_index][1],'\t',counter)
            print('%d > 1:%s' % (counter[0][1],counter[0][1] > 1))
            # 如果五个类别都是1,那么认为没有类别
            if counter[0][1] > 1:
                test_info_list[test_index][-1] = int(counter[0][0])
    ...
    def createDatasetWithKNN(self,extractImageFeature,k=5):
        # 目标: 或者直接使用knn最近邻算法，每个类别挑选出100张
        # 思路概述: 首先读取种子数据集，根据"类别名称-x"自动构建类别，每个类别中有5张图片，图片读取后使用某些算法提取特征
        seed_list = list()
        seed_info_list = list()
        type_label_map = dict()
        label_type_map = dict()
        label = 0
        # ?
        for root, dirs, files in os.walk(self.config['SEED_IMAGE_DIR']):
            for name in files:
                # 得到具体文件了，判断其后缀
                if IMAGE_SUFFIX in name:
                    ...
                    try:
                        ...
                        continue
                    #? 开始处理,首先对每个图像使用指定方法提取出特征(通过传递的函数)
                    #image_feature = extractImageFeature(image)
                    # 给类别编码
                    if dir_name not in type_label_map:
                        type_label_map[dir_name] = label
                        label_type_map[label] = dir_name
                        label += 1
                    # 以类别为键，直接取出一类的特征
                    '''
                    if dir_name not in seed_list[dir_name]:
                        seed_dict[dir_name] = dict()
                        seed_dict[dir_name]['paths'] = []
                        seed_dict[dir_name]['features'] = []
                    seed_dict[dir_name]['paths'].append( dir_name + os.path.sep + name )
                    seed_dict[dir_name]['features'].append(image_feature)
                    '''
                    #seed_list.append(image_feature + [ type_label_map[dir_name] ] )
                    # 将来用于进行特征提取
                    seed_list.append( absolute_path )
                    # 将来用于进行簇展示
                    seed_info_list.append( [dir_name + os.path.sep + name , type_label_map[dir_name] ])

        seed_list = extractImageFeature(seed_list)

        # 将所有数据集中的图片拿出，然后个比较然后取每个类别的前100个
        train_list = list()
        ...
        for root, dirs, files in os.walk(self.config['INPUT_DATA_DIR']):
            for name in files:
                ...
                if IMAGE_SUFFIX in name:
                    ...
                    #image_feature = extractImageFeature(image)
                    #-----------------开始比较特征
                    #train_list.append(image_feature + [0])
                    #train_list.append( np.concatenate( (image_feature,np.array([0])), axis=0 ) )
                    relative_path = '/'.join(absolute_path.split('/')[-4:])
                    app = relative_path.split('/')[0]

                    # 将来用于进行特征提取
                    train_list.append( absolute_path )
                    # 将来用于进行簇展示
                    train_info_list.append( [app, relative_path , -1])

        train_list = extractImageFeature(train_list)
        train_info_list = self.KNN(seed_list,seed_info_list,train_list,train_info_list) # 引用传参,train_list已经修改
            
        # 最后将矩阵提交给commitToDataBase即可
        self.commitToDataBase(train_list,train_info_list,label_type_map)

    def commitToDataBase(self,train_list,train_info_list,label_type_map):
        # [[feature,class],[feature,class]]
        self.writeVector(train_list,self.config['ICON_DATASET_PATH'])
        self.writeCSV(list(train_info_list),self.config['ICON_DATASET_INFO_PATH'])
        self.writeDict(label_type_map,self.config['ICON_DATASET_INFO_PATH'].replace('_info.csv','_label_type_map.json'))
...
#-------------------------------------------------------------
    ...
    def preparePrecluster(self,params):
        ...
        for root, dirs, files in os.walk(self.config['INPUT_DATA_DIR']):
            for name in files:
                # 得到具体文件了，判断其后缀
                if (IMAGE_SUFFIX in name) and (DROP_SUFFIX not in name):
                    print('---------------------------------------------')
                    absolute_path = root+os.path.sep+name
                    try:
    ...
    def runClassifing(self, operator=[], params=[]):
        ...
        if operator == 'createds':
            if params  == 'knn':
                self.createDatasetWithKNN(self.extractImageFeaturebyVGG)
            elif params == 'clustering':
                self.createDatasetWithClustering(self.extractImageFeature)
